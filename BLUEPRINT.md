...
>
> **Живая копия этой карты есть в самом приложении — блок «16 · Карта»** (строится из
> `CRM.registry`). Правила расширения см. в §11. При добавлении блока обнови и §11, и §3.
...
| Пользовательский блок | `custom.<id>` | **сам блок** (kind=custom) | никто (изолирован) |
...
## 3. КАРТА БЛОКОВ (15 канон + 16 мета + 17.. пользовательские)
...
| 16 | **map** (мета) | — | — | живая карта из `CRM.registry`; кнопка «+ Новый блок» (см. §11) |
| 17.. | пользовательские | `custom.<id>` | записи списка/текст | генерируются `CRM.customBlocks` |

> Нумерация и метаданные блоков теперь хранятся в `CRM.registry` (живой источник для блока 16).
> Канонические 15 блоков описаны в `CRM.CORE_META`. Блок 16 и пользовательские добавляются
> через `CRM.registerModule(...)` (§11) — он же проверяет правило «один владелец».
...
Тот же граф строится в приложении автоматически из `meta.writes` каждого блока (блок 16).
...
11. **Новый блок-владелец не может забрать чужую сущность** — `CRM.registerModule` бросает ошибку
    при конфликте владения (`CRM.ownerOf`). Пользовательский блок владеет только `custom.<id>`.

---

## 8. КОНВЕНЦИИ (НЕ ЛОМАТЬ)

1. Один HTML-файл, без сборки. Новый блок = **один вызов `CRM.registerModule(...)`** (он сам
   добавляет в `CRM.modules`, `CRM.TABS`, `CRM.ICONS`, `CRM.registry`). Руками три места не править.
...
> Текущий статус кода (на момент слоя расширяемости §11): `matches._create`, `promoteToTeam`,
> реальный `syncOppLineup` ещё НЕ внедрены — это следующий рефактор по §2/§10.1. Слой §11
> (registry/registerModule/ui/Карта/пользовательские блоки) уже готов и протестирован.

---

## 11. СЛОЙ РАСШИРЯЕМОСТИ (как безопасно добавлять блоки)
